...
X = data.drop(['label'], axis=1)
y = data['label']
X_train, X_test, y_train, y_test = train_test_split(X, y, train_size=0.6, test_size=0.4, random_state=random_seed)
...
all_actual = np.array(y_test)

# Save the numpy files
np.save('predict6040.npy', all_preds)
np.save('actual6040.npy', all_actual)
preds = np.load('predict6040.npy')
actual = np.load('actual6040.npy')
# Performance evaluation
cm = confusion_matrix(actual, preds)
tn, fp, fn, tp = cm[0, 0], cm[0, 1], cm[1, 0], cm[1, 1]
accuracy = accuracy_score(actual, preds)
